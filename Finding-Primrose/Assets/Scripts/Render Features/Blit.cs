...
public class Blit : ScriptableRendererFeature
{
    ...
    public class BlitPass : ScriptableRenderPass
    {
        ...
        //RenderTargetHandle m_TemporaryColorTexture; // - DeleteMe
        //RenderTargetHandle m_DestinationTexture; // - DeleteMe
        ...
        public BlitPass(RenderPassEvent renderPassEvent, BlitSettings settings, string tag)
        {
            ...
            m_ProfilerTag = tag;
            //m_TemporaryColorTexture.Init("_TemporaryColorTexture"); // - DeleteMe
            ...
            {
                //m_DestinationTexture.Init(settings.dstTextureId); // - DeleteMe
                m_DestinationTexture = RTHandles.Alloc(settings.dstTextureId, name: settings.dstTextureId);
            }
        }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if (settings.dstType == Target.TextureID)
            {
                //cmd.GetTemporaryRT(m_DestinationTexture.id, opaqueDesc, filterMode); // - DeleteMe
            ...
            {
                //cmd.GetTemporaryRT(m_TemporaryColorTexture.id, opaqueDesc, filterMode); // - DeleteMe
                cmd.GetTemporaryRT(Shader.PropertyToID(m_DestinationTexture.name), opaqueDesc, filterMode);

                //Blit(cmd, source, m_TemporaryColorTexture.Identifier(), blitMaterial, settings.blitMaterialPassIndex); // - DeleteMe
                cmd.Blit(source, m_TemporaryColorTexture, blitMaterial, settings.blitMaterialPassIndex);
                //Blit(cmd, m_TemporaryColorTexture.Identifier(), destination); // - DeleteMe
                cmd.Blit(m_TemporaryColorTexture, destination);
            }
            else
            {
                //Blit(cmd, source, destination, blitMaterial, settings.blitMaterialPassIndex); // - DeleteMe
        ...
        public override void FrameCleanup(CommandBuffer cmd)
        {
            if (settings.dstType == Target.TextureID)
            {
                //cmd.ReleaseTemporaryRT(m_DestinationTexture.id); // - DeleteMe
                cmd.ReleaseTemporaryRT(Shader.PropertyToID(m_DestinationTexture.name));
            }
            if (source == destination || (settings.srcType == settings.dstType && settings.srcType == Target.CameraColor))
            {
                //cmd.ReleaseTemporaryRT(m_TemporaryColorTexture.id); // - DeleteMe
    ...
    public override void SetupRenderPasses(ScriptableRenderer renderer, in RenderingData renderingData)
    {
        ...
        blitPass.Setup(src, dest); // - DeleteMe maybe?
    ...
    //    //blitPass.Setup(src, dest); // - DeleteMe
    //    renderer.EnqueuePass(blitPass);
    //}
}